...
def plt_scc_lif(bn, bv):
    ...
    ax_no_adap.set_xlabel("$k$", fontsize=utl.fontsize)
    ...
    for taun_no_adap, taun_weak_adap in zip(taus_no_adap, taus_weak_adap):
        data_no_adap = home + "/Data/LIF_network_noise/lif_green_noise_taua{:.2f}_delta{:.2f}_bv{:.2f}_bn{:.2f}_taun{:.2f}.dat".format(
        ...
        data_weak_adap = home + "/Data/LIF_network_noise/lif_green_noise_taua{:.2f}_delta{:.2f}_bv{:.2f}_bn{:.2f}_taun{:.2f}.dat".format(
    ...
    for n, taun in enumerate(tauns):
        data_file = home + "/Data/LIF_network_noise/lif_green_noise_taua{:.2f}_delta{:.2f}_bv{:.2f}_bn{:.2f}_taun{:.2f}.dat".format(
    ...
    plt.savefig(home + "/Data/Plots/fig8.pdf")
    plt.show()
