...
def plt_scc_lif_theory(mus, tau_a, delta_as, sigma, Dw):
    ...
    ax_prc1 = f.add_axes([x0, y0, width, height])
    ax_scc2 = f.add_axes([x0+width+w_spacer, y0 + height + h_spacer, width_scc, height])
    ax_scc1 = f.add_axes([x0+width+w_spacer, y0, width_scc, height])
    ax_cbar2 = f.add_axes([x0+width+width_scc+w_spacer+w_spacer_cbar, y0+ height + h_spacer, width_cbar, height])
    ax_cbar1 = f.add_axes([x0+width+width_scc+w_spacer+w_spacer_cbar, y0, width_cbar, height])

    axis = [ax_scc1, ax_scc2, ax_prc1, ax_prc2, ax_cbar1, ax_cbar2]
    axis_scc = [ax_scc1, ax_scc2]
    axis_prc = [ax_prc1, ax_prc2]
    axis_cbar = [ax_cbar1, ax_cbar2]

    # Set up plot layout
    ax_prc2.text(-0.4, 1.1, "(a)", size=10, weight = 'heavy', transform=ax_prc2.transAxes)
    ax_prc1.text(-0.4, 1.1, "(b)", size=10, weight = 'heavy', transform=ax_prc1.transAxes)
    ...
    for ax in axis_scc:
        ...
        ax.set_ylim(-0.9, 0.9)
    ax_scc1.set_xlabel("$k$", fontsize=utl.fontsize)
    for ax, t_det in zip(axis_prc, t_dets):
        ax.set_xticks([0, t_det/2 , t_det])
        ax.set_xticklabels([0, 0.5 , 1])
        ax.set_ylim([0, 0.7])
    ax_prc1.set_xlabel(r"$\tau/T^*$", fontsize=utl.fontsize)
    ...
    # Plot PRCs
    ax_prc2.set_title(r"$Z(\tau)$", fontsize=utl.fontsize)
    ...
    # Plot SCCs
    ax_scc2.set_title(r"$\rho_k$", fontsize=utl.fontsize)
    ...
    ax_cbar2.set_title(r"$\tau_\eta / T^*$", fontsize=utl.fontsize)
    ...
    ax_cbar1.set_yticklabels([r'$10^{{{:.0f}}}$'.format(np.log(exp)/np.log(10)) for exp in t_ratios])
    ax_cbar2.set_yticklabels([r'$10^{{{:.0f}}}$'.format(np.log(exp)/np.log(10)) for exp in t_ratios])

    #ax.legend()
    plt.savefig(home + "/Data/Plots_paper/4_lif_sccs.pdf".format(Dn), transparent=True)
    plt.show()
