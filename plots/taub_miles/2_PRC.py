...
if __name__ == "__main__":
    ...
    data = np.loadtxt(home + "/Data/Taub_miles/PRC.dat")
    ...
    ax11.set_xlim([0, 10.1715])

    data = np.loadtxt(home + "/CLionProjects/PhD/taub_miles_adap/out/PRC/taubs_miles_adap_PRC_t11.00_e0.10.dat")
    v, a, Ia, t = np.transpose(data)

    ax12.plot(t, v, c="k")
    ax12.grid(which='major', alpha=0.8, linestyle="--")
    ax12.tick_params(which="both", direction='in', labelsize=utl.labelsize)
    ax12.set_xlabel(r"$\tau$ / ms", fontsize=utl.fontsize)
    ax12.set_ylabel(r"$V$ / mV", fontsize=utl.fontsize)
    ax12.set_xlim([0, 10.1715])
    ...
    ISIs_adap = np.loadtxt(home + "/CLionProjects/PhD/taub_miles_adap/out/spikes_taubs_miles_adap.dat")
    ISIs_adap = ISIs_adap[100:]

    ISIs_cnoise = np.loadtxt(home + "/CLionProjects/PhD/taub_miles_adap/out/spikes_taubs_miles_cnoise.dat")
    ISIs_cnoise = ISIs_cnoise[100:]

    ISIs_adap_cnoise = np.loadtxt(home + "/CLionProjects/PhD/taub_miles_adap/out/spikes_taubs_miles_adap_cnoise_weak.dat")
    ...
    print(np.mean(ISIs_adap_cnoise))
    for k in ks:
    ...
    ax2.text(2.0, -0.15, r"$\rho_{k,a}$", ha="center", fontsize=utl.fontsize, c="C0")
    ax2.text(4.0, 0.10, r"$\rho_{k,\eta}$", ha="center", fontsize=utl.fontsize, c="C1")

    ax2.set_xticks([2, 4, 6, 8])
    ax2.set_ylim([-0.2, 0.4])
    ax2.legend(prop={"size": 7}, framealpha=1., edgecolor="k")
    plt.savefig(home + "/Data/Taub_miles/Plots/traub_miles_prc_and_scc.pdf")
    plt.show()
