...
def plt_qif_model(tau_a, tau_n, mu, delta, D):
    ...
    # set axis labels
    ax_vt.text(0.05, 1.0, "A", size=12, transform=ax_vt.transAxes, va="top")
    ax_pp.text(0.05, 1.0, "B", size=12, transform=ax_pp.transAxes, va="top")
    ax_prc.text(0.05, 1.0, "C", size=12, transform=ax_prc.transAxes, va="top")
    ax_scc.text(0.05, 1.0, "D", size=12, transform=ax_scc.transAxes, va="top")
    ...
    ax_prc.fill_between(np.linspace(0, t_det, 100), prc, 0, facecolor=st.colors[1], alpha=0.4, zorder=2)
    ...
    ax_scc.axhline(0, ls=":", c="C7", lw=1)
    ax_scc.scatter(k_range, corr_sim, s=20, ec="k", fc="w", label="sim.", zorder=2)
    ...
    ax_scc.legend(fancybox=False, prop={"size":7}, loc=1, ncol=2, framealpha=1., edgecolor="k")
    ...
    ax_vt.set_yticklabels([r"$-\pi$", r"$\pi$"])
    ax_vt.set_xticklabels([])
    ax_vt.plot(t_plt, [np.arctan(v) for v in v_plt], zorder=2, lw=1, c=st.colors[0])
    ax_vt.set_ylabel(r"$\theta$", fontsize=utl.fontsize)

    ax_at.plot(t_plt, a_plt, zorder=2, c=st.colors[2], lw = 1)
    ...
    ax_nt.plot(t_plt, eta_plt, zorder=2, lw = 1, c=st.colors[4])
    ax_nt.set_yticks([0])
    ...
    ax_pp.plot([np.arctan(v) for v in v_plt_pp], a_plt_pp, lw=1, c=st.colors[1])
    ax_pp.plot([np.arctan(v) for v in vt[:-2]], at[:-2], lw=2, c="k")
    ax_pp.plot([np.arctan(v) for v in vt[-3:]], at[-3:], lw=1, ls=":", c="k")
    ax_pp.scatter([np.pi / 2, -np.pi / 2], [at[-3], at[-1]], marker="s", s=20, ec="k", fc="w", zorder=4)
    ...
    ax_pp.set_xticklabels([r"$-\pi$", "$0$",r"$\pi$"])
    ...
    plt.savefig(home + "/Desktop/Presentations/SCC SfB/fig1.png", dpi=300)
    plt.show()
