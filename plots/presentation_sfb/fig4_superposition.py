...
def plt_special_cases():
    ...
    for i, ax in enumerate(axis):
        ...
        ax.scatter(k_range, scc_adap_sim, ec=st.colors[2], fc="w",  s=20, zorder=2)
        ax.scatter(k_range, scc_cnoise_sim, ec=st.colors[4], fc="w", s=20, zorder=2)
        ax.plot(k_range, scc_theory, label="theory", c="k", ls="-", lw=1, zorder=1)
        ax.plot(k_range, scc_adap_theory, c=st.colors[2], ls="-", lw=1, zorder=1)
        ax.plot(k_range, scc_cnoise_theory, c=st.colors[4], ls="-", lw=1, zorder=1)
        ax.axhline(0, xmin=0, xmax=6, ls=":", c="C7", zorder=1)
    ax_l.text(0.1, 0.9, "A", size=12, transform=ax_l.transAxes)
    ax_r.text(0.1, 0.9, "B", size=12, transform=ax_r.transAxes)
    ax_l.text(2.0, -0.25, r"$\rho_{k,a}$", fontsize=15, ha="center", c=st.colors[2])
    ax_l.text(3.5, 0.08, r"$\rho_{k,\eta}$", fontsize=15, ha="center",  c=st.colors[4])
    ax_r.text(3.5, -0.2, r"$\rho_{k,a}$", fontsize=15, ha="center", c=st.colors[2])
    ax_r.text(4.5, 0.65, r"$\rho_{k,\eta}$", fontsize=15, ha="center", c=st.colors[4])
    ax_l.legend(fancybox=False, prop={"size": 7}, framealpha=1., edgecolor="k")
    plt.savefig(home + "/Desktop/Presentations/SCC SfB/fig4.png", dpi=300)
    plt.show()
